...
void reb_integrator_sei_init(struct reb_simulation* const r){
     ...
     */
    r->ri_sei.sindt = sin(r->ri_sei.OMEGA*r->ri_sei.KAPPA_OMEGA*(-r->dt/2.)); // ACQ
    r->ri_sei.tandt = tan(r->ri_sei.OMEGA*r->ri_sei.KAPPA_OMEGA*(-r->dt/4.));
...
static void operator_H012(double dt, const struct reb_simulation_integrator_sei ri_sei, struct reb_particle* p){
	...
	p->vz = zyt;
        ...
        const double om_ka2 = 1.0/(ri_sei.KAPPA_OMEGA*ri_sei.KAPPA_OMEGA);
        const double fourmkap3= (4.0 - ri_sei.KAPPA_OMEGA*ri_sei.KAPPA_OMEGA)/3.0;
	// Integrate motion in xy directions
        // Center of epicyclic motion (guiding center) 
	const double aO = (2.*p->vy   + 4.*p->x*ri_sei.OMEGA)*om_ka2;	
	const double bO = p->y*ri_sei.OMEGA - 2.*p->vx*om_ka2;
        // ACQ a0 = x_guiding *Omega
        // ACQ b0 = y_guiding *Omega

        // Epicycle vector (amplitude)
	const double ys = (p->y*ri_sei.OMEGA-bO)/2.*ri_sei.KAPPA_OMEGA; 	
	...
	p->y  = (yst/ri_sei.KAPPA_OMEGA*2.+bO) /ri_sei.OMEGA - 3./4.*fourmkap3*aO*dt;	
        // step is dt/2
	p->vx = yst*ri_sei.KAPPA_OMEGA;  // FIX
	p->vy = -xst*2. -  3./2.*fourmkap3*aO;
}
