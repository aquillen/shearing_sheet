...
void reb_boundary_check(struct reb_simulation* const r){
	...
	switch(r->boundary){
		...
		case REB_BOUNDARY_SHEAR:
		{
			...
			const double KAPPA_OMEGA = r->ri_sei.KAPPA_OMEGA;
                        const double fourmkap3=(4.0-KAPPA_OMEGA*KAPPA_OMEGA)/3.0;
                        // ACQ
			const double offsetp1 = -fmod(-3.0/2.0*fourmkap3*OMEGA*boxsize.x*r->t+boxsize.y/2.,boxsize.y)-boxsize.y/2.; 
			const double offsetm1 = -fmod( 3.0/2.0*fourmkap3*OMEGA*boxsize.x*r->t-boxsize.y/2.,boxsize.y)+boxsize.y/2.; 
// is this correct? ACQ? perhaps no factor of fourmkap3?
			struct reb_particle* const particles = r->particles;
#pragma omp parallel for schedule(guided)
			for (int i=0;i<N;i++){
				...
				while(particles[i].x>boxsize.x/2.){
					...
					particles[i].vy += 3./2.*fourmkap3*OMEGA*boxsize.x;
				}
				while(particles[i].x<-boxsize.x/2.){
					particles[i].x += boxsize.x;
...
struct reb_ghostbox reb_boundary_get_ghostbox(struct reb_simulation* const r, int i, int j, int k){
	switch(r->boundary){
		...
		case REB_BOUNDARY_SHEAR:
		{
			...
			gb.shiftvx = 0.;
			gb.shiftvy = -3.0/2.0*fourmkap3*(double)i*OMEGA*r->boxsize.x;
                        // ACQ
			gb.shiftvz = 0.;
